...
def generate_card_image(text):
    ...
    for i, (words, line_w) in enumerate(wrapped_lines):
        if i < len(wrapped_lines) - 1 and len(words) > 1 and line_w < max_text_width:
            ...
            draw.text((CARD_PADDING, text_y), ' '.join(words), font=font, fill=TEXT_COLOR)
        text_y += line_height
    ...
    footer_x = (CARD_WIDTH - fb_w) // 2
    draw.text((footer_x, sep_y + gap_after_sep), footer_full, font=footer_font, fill=FOOTER_COLOR)
