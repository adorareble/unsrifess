...
def generate_card_image(text):
    ...
    wrapped_lines = _wrap_text(text, font, max_text_width, draw_d)
    ...
    fb = draw_d.textbbox((0, 0), footer_full, font=footer_font)
    footer_text_h = fb[3] - fb[1]
    gap_after_text = 24
    gap_after_sep = 14

    content_block_h = 30 + text_block_height + gap_after_text + 1 + gap_after_sep + footer_text_h
    ...
    draw.text((CARD_PADDING, content_origin - 15), '"', font=quote_font, fill=QUOTE_COLOR)

    text_y = content_origin + 30
    for line in wrapped_lines:
        draw.text((CARD_PADDING, text_y), line, font=font, fill=TEXT_COLOR)
        text_y += line_height

    sep_y = content_origin + 30 + text_block_height + gap_after_text
    draw.line(
        [(CARD_PADDING, sep_y), (CARD_WIDTH - CARD_PADDING, sep_y)],
        fill=SEP_COLOR, width=1,
    )

    draw.text((CARD_PADDING, sep_y + gap_after_sep), footer_full, font=footer_font, fill=FOOTER_COLOR)
